...
The core hardware of the UAV IRIS+ is the Pixhawk and we can control it using an API for Android \cite{dronekit}, which uses Micro Air Vehicle Link (MAVLink) protocol~\cite{mavilink}. MAVLink is a protocol for communicating with small unmanned vehicle, which is designed as a header-only message marshalling library.
